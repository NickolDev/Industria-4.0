...
async function log(usuario, acao, tabela, detalhe) {
  try {
    await pool.query(
      'INSERT INTO auditoria (usuario,acao,tabela,detalhe) VALUES ($1,$2,$3,$4)',
      [usuario, acao, tabela, detalhe]
    );
  } catch {}
}
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    await log(u.nome, 'LOGIN', 'usuarios', `Login realizado — perfil: ${u.perfil}`);
...
app.post('/api/usuarios', auth, async (req, res) => {
  ...
  try {
    const r = await pool.query(
      'INSERT INTO usuarios (nome,usuario,senha,perfil) VALUES ($1,$2,$3,$4) RETURNING id',
      [nome, usuario, senha, perfil || 'atendente']
    );
    await log(req.user.nome, 'CRIAR', 'usuarios', `Usuário criado: ${usuario}`);
    res.json({ id: r.rows[0].id });
  } catch(e) { res.status(400).json({ erro: 'Usuário já existe' }); }
});

app.put('/api/usuarios/:id', auth, async (req, res) => {
  const { nome, usuario, senha, perfil } = req.body;
  if (senha) {
    await pool.query('UPDATE usuarios SET nome=$1,usuario=$2,senha=$3,perfil=$4 WHERE id=$5', [nome, usuario, senha, perfil, req.params.id]);
  } else {
    await pool.query('UPDATE usuarios SET nome=$1,usuario=$2,perfil=$3 WHERE id=$4', [nome, usuario, perfil, req.params.id]);
  }
  await log(req.user.nome, 'EDITAR', 'usuarios', `Usuário editado: ${usuario}`);
  res.json({ ok: true });
});
...
app.get('/api/clientes', auth, async (req, res) => {
  ...
  const r = await pool.query(`
    SELECT c.*, COUNT(v.id)::int as total_veiculos,
...
app.get('/api/clientes/:id', auth, async (req, res) => {
  ...
  if (!cli.rows.length) return res.status(404).json({ erro: 'Cliente não encontrado' });
  const veics = await pool.query('SELECT * FROM veiculos WHERE cliente_id=$1', [req.params.id]);
  const os    = await pool.query(`
    SELECT os.*, v.placa, v.modelo
    FROM ordens_servico os JOIN veiculos v ON v.id=os.veiculo_id
    WHERE os.cliente_id=$1 ORDER BY os.id DESC LIMIT 20
  `, [req.params.id]);
...
app.post('/api/clientes', auth, async (req, res) => {
  ...
  const r = await pool.query(
    'INSERT INTO clientes (nome,telefone,email,cpf,endereco) VALUES ($1,$2,$3,$4,$5) RETURNING id',
    [nome, telefone, email, cpf, endereco]
  );
  await log(req.user.nome, 'CRIAR', 'clientes', `Cliente criado: ${nome}`);
  res.json({ id: r.rows[0].id });
});

app.put('/api/clientes/:id', auth, async (req, res) => {
  const { nome, telefone, email, cpf, endereco } = req.body;
  await pool.query('UPDATE clientes SET nome=$1,telefone=$2,email=$3,cpf=$4,endereco=$5 WHERE id=$6',
    [nome, telefone, email, cpf, endereco, req.params.id]);
  await log(req.user.nome, 'EDITAR', 'clientes', `Cliente editado: ${nome}`);
  res.json({ ok: true });
});
...
app.post('/api/veiculos', auth, async (req, res) => {
  ...
  if (!cliente_id || !placa || !modelo) return res.status(400).json({ erro: 'Cliente, placa e modelo são obrigatórios' });
  try {
    const r = await pool.query(
      'INSERT INTO veiculos (cliente_id,placa,modelo,marca,ano,cor,km_atual) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id',
      [cliente_id, placa.toUpperCase(), modelo, marca, ano, cor, km_atual || 0]
    );
    await log(req.user.nome, 'CRIAR', 'veiculos', `Veículo cadastrado: ${placa}`);
    res.json({ id: r.rows[0].id });
...
app.put('/api/veiculos/:id', auth, async (req, res) => {
  ...
  await pool.query('UPDATE veiculos SET placa=$1,modelo=$2,marca=$3,ano=$4,cor=$5,km_atual=$6 WHERE id=$7',
    [placa.toUpperCase(), modelo, marca, ano, cor, km_atual, req.params.id]);
  res.json({ ok: true });
});
...
app.post('/api/pecas', auth, async (req, res) => {
  ...
  if (!nome) return res.status(400).json({ erro: 'Nome é obrigatório' });
  const r = await pool.query(
    'INSERT INTO pecas (nome,categoria,unidade,qtd,qtd_minima,fornecedor,preco_custo) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id',
    [nome, categoria || 'Outros', unidade || 'unid.', qtd || 0, qtd_minima || 1, fornecedor, preco_custo || 0]
  );
  res.json({ id: r.rows[0].id });
});
...
app.put('/api/pecas/:id', auth, async (req, res) => {
  const { nome, categoria, unidade, qtd, qtd_minima, fornecedor, preco_custo } = req.body;
  await pool.query('UPDATE pecas SET nome=$1,categoria=$2,unidade=$3,qtd=$4,qtd_minima=$5,fornecedor=$6,preco_custo=$7 WHERE id=$8',
    [nome, categoria, unidade, qtd, qtd_minima, fornecedor, preco_custo, req.params.id]);
  res.json({ ok: true });
});
...
app.patch('/api/pecas/:id/ajustar', auth, async (req, res) => {
  ...
  const r = await pool.query(
    'UPDATE pecas SET qtd=GREATEST(0,qtd+$1) WHERE id=$2 RETURNING qtd',
    [delta, req.params.id]
  );
...
app.get('/api/elevadores', auth, async (req, res) => {
  const r = await pool.query(`
    SELECT e.*,
      ...
      os.status as os_status, os.criado_em as os_criado_em,
      c.nome as cliente_nome, v.placa as veiculo_placa,
      EXTRACT(EPOCH FROM (NOW()-e.ocupado_em))/60 as minutos_ocupado
    FROM elevadores e
    LEFT JOIN ordens_servico os ON os.id=e.os_id AND os.status NOT IN ('concluido','cancelado')
...
app.patch('/api/elevadores/:id/liberar', auth, async (req, res) => {
  ...
  await log(req.user.nome, 'LIBERAR', 'elevadores', `Elevador ${req.params.id} liberado manualmente`);
  res.json({ ok: true });
});
...
app.get('/api/os', auth, async (req, res) => {
  ...
  if (busca) {
    sql += ` AND (c.nome ILIKE $${i} OR v.placa ILIKE $${i} OR os.numero ILIKE $${i})`;
    params.push(`%${busca}%`); i++;
  }
...
app.get('/api/os/:id', auth, async (req, res) => {
  ...
  const pecas = await pool.query(`
    SELECT op.*, p.nome as peca_nome, p.unidade
...
app.post('/api/os', auth, async (req, res) => {
  ...
  if (!cliente_id || !veiculo_id || !mecanico || !servico)
    return res.status(400).json({ erro: 'Cliente, veículo, mecânico e serviço são obrigatórios' });
  ...
  try {
    ...
    if (elevador_id) {
      ...
      if (elev.rows[0]?.status === 'ocupado')
        return res.status(400).json({ erro: `${elev.rows[0].nome} já está ocupado` });
    }
    const numero = await gerarNumero(client);
    const r = await client.query(
      'INSERT INTO ordens_servico (numero,cliente_id,veiculo_id,elevador_id,mecanico,servico,obs,km_entrada,valor_total) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id',
    ...
    await log(req.user.nome, 'CRIAR', 'ordens_servico', `OS criada: ${numero}`);
...
app.put('/api/os/:id', auth, async (req, res) => {
  ...
  await pool.query(
    'UPDATE ordens_servico SET mecanico=$1,servico=$2,obs=$3,status=$4,elevador_id=$5,km_entrada=$6,valor_total=$7 WHERE id=$8',
...
app.patch('/api/os/:id/status', auth, async (req, res) => {
  ...
  try {
    ...
    if (status === 'aguardando' && o.elevador_id)
...
app.get('/api/relatorios/os', auth, async (req, res) => {
  ...
  let sql = `
    SELECT os.numero, os.criado_em, os.concluido_em, os.status,
      os.servico, os.mecanico, os.valor_total, os.km_entrada,
      c.nome as cliente, c.telefone, v.placa, v.modelo
    FROM ordens_servico os
    JOIN clientes c ON c.id=os.cliente_id
    JOIN veiculos v ON v.id=os.veiculo_id WHERE 1=1
  `;
...
app.get('/api/relatorios/mecanicos', auth, async (req, res) => {
  const r = await pool.query(`
    SELECT mecanico,
      ...
      COUNT(CASE WHEN status='concluido' THEN 1 END)::int as concluidas,
      COUNT(CASE WHEN status='andamento' THEN 1 END)::int as em_andamento,
      SUM(CASE WHEN status='concluido' THEN valor_total ELSE 0 END)::numeric as faturamento
...
app.get('/api/dashboard', auth, async (req, res) => {
  const [and,ag,mu,co,clis,veics,pecrit,elevbloq,osrec] = await Promise.all([
    pool.query("SELECT COUNT(*)::int as n FROM ordens_servico WHERE status='andamento'"),
    pool.query("SELECT COUNT(*)::int as n FROM ordens_servico WHERE status='aguardando'"),
    pool.query("SELECT COUNT(*)::int as n FROM ordens_servico WHERE status='multidias'"),
    pool.query("SELECT COUNT(*)::int as n FROM ordens_servico WHERE status='concluido'"),
    ...
    pool.query(`SELECT os.numero,os.status,os.criado_em,c.nome as cliente,v.placa
  ...
  res.json({
    os_andamento:   and.rows[0].n,
    os_aguardando:  ag.rows[0].n,
    os_multidias:   mu.rows[0].n,
    os_concluidas:  co.rows[0].n,
    total_clientes: clis.rows[0].n,
    total_veiculos: veics.rows[0].n,
    pecas_criticas: pecrit.rows[0].n,
    elev_bloqueados:elevbloq.rows[0].n,
    os_recentes:    osrec.rows,
  });
});
